...
import { getLocalStorage, gotoEntURL } from "@/common/utils/index";
...
export default {
    ...
    created() {
        this.urls.forEach(v => {
            if (v.id == 85) {
                if (getLocalStorage("unitlevel") == 3) {
                    //企业端跳详情页
                    v.url = `entDetail.html?entName=${getLocalStorage("name")}`;
                }
            }
        });
    },
