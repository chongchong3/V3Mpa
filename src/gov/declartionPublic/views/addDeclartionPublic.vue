...
export default {
  ...
  methods: {
    ...
    editData(id) {
      inter.getOneDeclareResult({ id }).then(response => {
        if (response.code === "0000") {
    ...
    suresEvent(status) {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.formData.fileName = files.map(item => item.response.data).join(",");
          inter.insertDeclareResult(this.formData).then(response => {
            if (response.code == "0000") {
              this.$router.push("/declartionPublicList");
            }
          });
        } else {
